...
class Raft(raft_pb2_grpc.RaftServicer):
    ...
    def save_final(self):
        with open(self.metadata, "w") as f:
            f.write(
                f"{self.currentTerm} {self.votedFor} {self.commitLength} {self.leaderID}\n")
...
def serve():
    ...
    try:
        ...
        with open(raft_node.metadata, "r") as f:
            ...
            raft_node.currentTerm = int(line[0])
            raft_node.votedFor = int(line[1])
            raft_node.commitLength = int(line[2])
            raft_node.leaderID = int(line[3])
