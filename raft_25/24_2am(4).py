...
class Raft(raft_pb2_grpc.RaftServicer):
    def __init__(self, id, ip_port, nodes):
        ...
        self.heartbeatTimeout = 2  # Heartbeat timeout in seconds
    ...
    def start_election(self):
        ...
        for node in self.nodes:
            if node != self.ip_port:
                threading.Thread(target=self.request_vote, args=(node, msg)).start()
    ...
    def RequestVote(self, request, context):
        print(f"RequestVote received from {request.candidate_id} for term {request.term}")
        ...
        if request.term == self.currentTerm and logOk and (self.votedFor is None or self.votedFor == request.candidate_id):
            ...
            self.reset_election_timeout()
            return raft_pb2.RequestVoteReply(term=self.currentTerm, vote_granted=True)
        return raft_pb2.RequestVoteReply(term=self.currentTerm, vote_granted=False)
    ...
    def handle_vote_response(self, response):
        ...
        if response.term > self.currentTerm:
            ...
            with self.vote_lock:
                ...
                self.voteCount += 1
                self.leader_lease = max(self.leader_lease, response.remaining_lease)
        ...
        if self.currentRole == "Leader":
            ...
            if request_formatted == "SET":
                ...
                if value is None:
                    ...
                    print(f"GET request processed for key: {key}, value: {value}")
    ...
    def broadcast_message(self, msg):
        if self.currentRole == "Leader":
            log_entry = raft_pb2.LogEntry(term=self.currentTerm, operation=msg, index=len(self.logs) + 1)
            ...
            if self.leaderID is not None:
                ...
                stub = raft_pb2_grpc.RaftStub(channel)
                try:
                    response = stub.ServeClient(raft_pb2.ServeClientArgs(Request=msg))
    ...
    def ReplicateLog(self, follower):
        ...
        try:
            ...
            print(f"Unable to connect to node: {follower}. It is detected to be down.")
    ...
    def AppendEntries(self, request, context):
        ...
        logOk = (len(self.logs) >= prefixLen) and (prefixLen == 0 or self.logs[prefixLen - 1].term == request.prev_log_term)
        self.leader_lease = request.remaining_lease
        if request.term == self.currentTerm and logOk:
            self.AppendEntries_helper(prefixLen, request.leader_commit, request.entries)
        ...
        if response.term > self.currentTerm:
            ...
            if response.success and response.term == self.currentTerm:
                ...
                if len(self.ackCount) >= (len(self.nodes) + 1) // 2:    
                    self.ackedLength[follower] = max(self.ackedLength[follower], len(self.logs))
                    self.leader_lease = leader_lease_time
    ...
    def CommitLogEntries(self):
        ...
        acks = {node: length for node, length in self.ackedLength.items() if length >= len(self.logs)}
